...
class ProjectDetails{
	...
	public function getRemarks(){
		...
		public function setExctime($exctime){
...
class ProjectDetailsDAO{
	...
	public function check_project_details_status_by_user_id_and_project_id($criteria){
        ...
        $SQL_READ_ALL = "SELECT * FROM ".ProjectDetails::$TABLE_NAME." WHERE (user_id=:user_id AND is_completed!=2 AND project_id=:project_id) OR (delegated_id=:delegated_id AND is_completed!=2 AND project_id=:project_id)";
		...
		$user_id=$projectDetails_obj->getUserId();
		
		$priority=$projectDetails_obj->getPriority();
		
		$overdue=$projectDetails_obj->getOverdue();

		$SQL_FIND = "SELECT * FROM ".ProjectDetails::$TABLE_NAME." WHERE project_id=:project_id AND is_completed NOT IN(2,5)";
		
		$database = new Database();

		if($user_id!= ''){
    		$SQL_FIND .= "AND user_id='$user_id'";
		//$database->bind(':user_id',$projectDetails_obj->getUserId());

		}
		if($priority!= ''){
    		$SQL_FIND .= "AND priority='$priority'";
		//$database->bind(':user_id',$projectDetails_obj->getUserId());

		}
		
		if($overdue== ''){
    		$SQL_FIND .= " AND task_end_date>=CURDATE()";
		}
		...
		foreach ($records as $row){
				...
				$projectDetailsObj->setExctime($timetaken);
		...
		return $count;
			...
			public function getTotalOverDueTaskDetailsByProjectIdAndUserId($projectDetails_obj){

    	$SQL_COUNT = "SELECT id,project_id,task_name,task_description,task_start_date,task_end_date,datediff(CURDATE(),task_end_date) as overdue FROM 
    	".ProjectDetails::$TABLE_NAME." where CURDATE() > task_end_date and is_completed=1 and user_id=:user_id and project_id=:project_id";
    	...
    	return 0;
					...
					public function getTotalOverDueTaskDetailsByProjectId($projectDetails_obj){

    	$SQL_COUNT = "SELECT id,project_id,task_name,task_description,task_start_date,task_end_date,datediff(CURDATE(),task_end_date) as overdue FROM 
    	".ProjectDetails::$TABLE_NAME." where CURDATE() > task_end_date and is_completed=1 and project_id=:project_id";
